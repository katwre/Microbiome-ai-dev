...
out <- filterAndTrim("sampleID_2a.trimmed_1.trim.fastq.gz", "sampleID_2a_1.filt.fastq.gz", "sampleID_2a.trimmed_2.trim.fastq.gz", "sampleID_2a_2.filt.fastq.gz",
    truncLen = c(230, 229),
...
if(out[2] == '0'){
    for(fp in c("sampleID_2a_1.filt.fastq.gz", "sampleID_2a_2.filt.fastq.gz")){
...
write.table( out, file = "sampleID_2a.filter_stats.tsv", sep = "\t", row.names = FALSE, quote = FALSE, na = '')
