#!/usr/bin/env Rscript
    ...
    if ( sum_readfiles_length > 2147483647 ) {
        ...
        write.table(max_files, file = paste0(max_files," files were used for RV plotQualityProfile.txt"), row.names = FALSE, col.names = FALSE, quote = FALSE, na = '')
    }
    ...
    #write output
    write.table( t(df), file = paste0("RV_qual_stats",".tsv"), sep = "	", row.names = TRUE, col.names = FALSE, quote = FALSE)
    pdf(paste0("RV_qual_stats",".pdf"))
    plot
    dev.off()
    svg(paste0("RV_qual_stats",".svg"))
    plot
    dev.off()

    write.table(paste0('plotQualityProfile	, n = 5e+04, aggregate = TRUE
max_files	',max_files), file = "RV_plotQualityProfile.args.txt", row.names = FALSE, col.names = FALSE, quote = FALSE, na = '')
