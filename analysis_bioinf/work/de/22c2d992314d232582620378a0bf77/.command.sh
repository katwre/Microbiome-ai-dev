...
qiime taxa collapse \
    ...
    --p-level 4 \
    --o-collapsed-table "lvl4-treatment1.qza"

# Extract summarised table and output a file with the number of taxa
qiime tools export \
    --input-path "lvl4-treatment1.qza" \
    --output-path exported/
biom convert \
    -i exported/feature-table.biom \
    -o "lvl4-treatment1.feature-table.tsv" \
    --to-tsv

if [ $(grep -v '^#' -c "lvl4-treatment1.feature-table.tsv") -lt 2 ]; then
    mkdir differentials
    ...
    qiime composition ancombc \
        --i-table "lvl4-treatment1.qza" \
        --m-metadata-file "Metadata.tsv" \
        --p-prv-cut 0.1 --p-lib-cut 500 --p-alpha 0.05 --p-conserve \
        --p-formula 'treatment1' \
        --o-differentials "lvl4-treatment1.differentials.qza" \
        --verbose
    qiime tools export \
        --input-path "lvl4-treatment1.differentials.qza" \
        --output-path "differentials/Category-treatment1-level-4"

    # Generate tabular view of ANCOM-BC output
    qiime composition tabulate \
        --i-data "lvl4-treatment1.differentials.qza" \
        --o-visualization "lvl4-treatment1.differentials.qzv"
    qiime tools export \
        --input-path "lvl4-treatment1.differentials.qzv" \
        --output-path "differentials/Category-treatment1-level-4"
    ...
    qiime composition da-barplot \
        ...
        --o-visualization "lvl4-treatment1.da_barplot.qzv"
    qiime tools export --input-path "lvl4-treatment1.da_barplot.qzv" \
        --output-path "da_barplot/Category-treatment1-level-4"
fi
