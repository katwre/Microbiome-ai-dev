#!/usr/bin/env Rscript
    ...
    if ( sum_readfiles_length > 2147483647 ) {
        ...
        write.table(max_files, file = paste0("WARNING Only ",max_files," of ",length(readfiles)," files and ",sum(readfiles_length[1:max_files])," of ",sum_readfiles_length," reads were used for RV plotQualityProfile.txt"), row.names = FALSE, col.names = FALSE, quote = FALSE, na = '')
        readfiles <- readfiles[1:max_files]
    } else {
        max_files <- length(readfiles)
        write.table(max_files, file = paste0(max_files," files were used for RV_preprocessed plotQualityProfile.txt"), row.names = FALSE, col.names = FALSE, quote = FALSE, na = '')
    }
    ...
    #write output
    write.table( t(df), file = paste0("RV_preprocessed_qual_stats",".tsv"), sep = "	", row.names = TRUE, col.names = FALSE, quote = FALSE)
    pdf(paste0("RV_preprocessed_qual_stats",".pdf"))
    plot
    dev.off()
    svg(paste0("RV_preprocessed_qual_stats",".svg"))
    plot
    dev.off()

    write.table(paste0('plotQualityProfile	, n = 5e+04, aggregate = TRUE
max_files	',max_files), file = "RV_preprocessed_plotQualityProfile.args.txt", row.names = FALSE, col.names = FALSE, quote = FALSE, na = '')
    writeLines(c("\"NFCORE_AMPLISEQ:AMPLISEQ:DADA2_PREPROCESSING:DADA2_QUALITY2\":", paste0("    R: ", paste0(R.Version()[c("major","minor")], collapse = ".")),paste0("    dada2: ", packageVersion("dada2")),paste0("    ShortRead: ", packageVersion("ShortRead")) ), "versions.yml")
